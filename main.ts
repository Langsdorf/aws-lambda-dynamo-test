...
app.post("/employees", async (req, res) => {
  ...
  if (checkFields(id, name, age, role))
  ...
  try {
    ...
    generateResponse(res, 500, "Could not create employees", e);
  }
});
...
app.delete("/employees/:uuid", async (req, res) => {
  try {
    ...
    generateResponse(res, 500, "Could not delete employees", e);
  }
});
  ...
  try {
    const result = await dynamoDb({
      ...
      params: {
        ...
        UpdateExpression:
          "set id = :id, name = :name, age = :age, role = :role",
